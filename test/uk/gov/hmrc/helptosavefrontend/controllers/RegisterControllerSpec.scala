...
class RegisterControllerSpec extends TestSupport {
  ...
  "The RegisterController" when {
    ...
    "checking eligibility" must {
      ...
      "If the outgoing-json validateOutGoingJson function detects a forename with too few characters it returns a left " in {
        val noForenameUser = validNSIUserInfo copy (forename = "")
        val result = register.validateOutGoingUserInfo(Some(noForenameUser))
