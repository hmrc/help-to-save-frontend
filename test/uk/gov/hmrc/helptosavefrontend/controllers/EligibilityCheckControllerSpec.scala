    ...
    "checking eligibility" when {
      ...
      "the user is not already enrolled" must {
        ...
        "show the user a page informing them which fields of their user info are missing" in {
          ...
          def missingUserInfoRetrieval(name: Option[String], surname: Option[String], dob: Option[LocalDate], address: ItmpAddress):
          ~[~[~[~[~[~[Name, Option[String]], Option[LocalDate]], ItmpName], Option[LocalDate]], ItmpAddress], Option[String]] =
            new ~(Name(name, surname), email) and dob and ItmpName(name, None, surname) and dob and address and mockedNINORetrieval

          val itmpAddress2 = ItmpAddress(None, Some(line2), Some(line3), None, None, Some(postCode), Some(countryCode), Some(countryCode))
          val itmpAddress3 = ItmpAddress(Some(line1), None, Some(line3), None, None, Some(postCode), Some(countryCode), Some(countryCode))

          def mockedGeneratedMissingUserInfo(): List = {
            val userInfo1 = missingUserInfoRetrieval(Some(firstName), Some(lastName), Some(dob), itmpAddress)
            val userInfoNoFirstName = missingUserInfoRetrieval(None, Some(lastName), Some(dob), itmpAddress)
            val userInfoNoLastName = missingUserInfoRetrieval(Some(firstName), None, Some(dob), itmpAddress)
            val userInfoNoDob = missingUserInfoRetrieval(Some(firstName), Some(lastName), None, itmpAddress)
            val userInfoNoAddressLine1 = missingUserInfoRetrieval(Some(firstName), Some(lastName), Some(dob), itmpAddress2)
            val userInfoNoAddressLine2 = missingUserInfoRetrieval(Some(firstName), Some(lastName), Some(dob), itmpAddress3)

            val x = List(userInfo1, userInfoNoFirstName, userInfoNoLastName, userInfoNoDob, userInfoNoAddressLine1, userInfoNoAddressLine2)
          }

          case class TestParameters(name: Option[String], surname: Option[String], dob: Option[LocalDate], address: ItmpAddress)
          ...
          val gen: Gen[TestParameters] = for{
            ...
            dob ← Gen.option(Gen.choose(0,10).map(LocalDate.now().minusDays))
          ...
          forAll(gen){ params ⇒
            ...
            def isInvalid(address: ItmpAddress): Boolean
            ...
            html.contains("Name") shouldBe params.name.isEmpty
            html.contains("Surname") shouldBe params.surname.isEmpty
            html.contains("Date of Birth") shouldBe params.dob.isEmpty
            html.contains("Address") shouldBe isInvalid(params.address)
          }
