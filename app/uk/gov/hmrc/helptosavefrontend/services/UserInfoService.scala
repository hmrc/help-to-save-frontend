...
class UserInfoService(citizenDetailsConnector: CitizenDetailsConnector) extends ServicesConfig {
  ...
  def getUserInfo(userDetailsResponse: UserDetailsResponse, nino: NINO)(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[UserInfo] =
    for {
      userDetails ← queryUserDetails(userDetailsResponse)
  ...
  private def queryUserDetails(userDetailsResponse: UserDetailsResponse)(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[UserDetailsResponse] =
    EitherT.right[Future, String, UserDetailsResponse](Future.successful(userDetailsResponse))
