...
class HelpToSaveReminderConnectorImpl @Inject()(http: HttpClientV2)(implicit frontendAppConfig: FrontendAppConfig)
  ...
  override def updateHtsUser(
    ...
    handle(
      ...
      _.parseJSON[HtsUserSchedule](), "update htsUser")
  ...
  override def updateReminderEmail(
    ...
    handle(
      ...
      _ => Right(()), "update email")

  override def cancelHtsUserReminders(
    cancelHtsUserReminder: CancelHtsUserReminder
  )(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[Unit] = {

    val result =  http.post(url"$cancelHtsReminderURL").withBody(Json.toJson(cancelHtsUserReminder)).execute[Either[UpstreamErrorResponse,HttpResponse]]


    for {
      response <- toEitherT("cancel reminder",
        result)
      result <- response match {
        case Right(_) => EitherT.rightT[Future, String](())
        case  Left(response) if response.statusCode == NOT_MODIFIED => EitherT.rightT[Future, String](())
        case Left(response) =>
                       EitherT.leftT[Future, Unit](
                   s"Call to 'cancel reminder' came back with status ${response.statusCode}. Body was ${response.getMessage()}")
               }
    } yield result
  }
