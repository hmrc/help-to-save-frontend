...
class WhitelistFilter(configuration: Configuration, materializer: Materializer) extends AkamaiWhitelistFilter {

  override def whitelist: Seq[String] =
    configuration.underlying.get[List[String]]("http-header-ip-whitelist").value

  override def destination: Call = Call("GET", "not-allowed")
