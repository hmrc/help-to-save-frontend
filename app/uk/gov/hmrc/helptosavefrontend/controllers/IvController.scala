...
import uk.gov.hmrc.helptosavefrontend.models.IvSuccessResponse._
        ...
        journeyId match {
          case Some(id) ⇒
            ivConnector.getJourneyStatus(JourneyId(id)).map {
              ...
              case IvSuccessResponse(Success) ⇒
                Ok(success.iv_success(HtsDeclarationUrl))

              case IvSuccessResponse(Incomplete) ⇒
              ...
              case IvSuccessResponse(FailedMatching) ⇒
              ...
              case IvSuccessResponse(FailedIV) ⇒
                //The user couldn't answer enough questions correctly to pass verification
                Unauthorized(failed_matching(ivRetryUrl))

              case IvSuccessResponse(InsufficientEvidence) ⇒
              ...
              case IvSuccessResponse(TechnicalIssue) ⇒
              ...
              case IvSuccessResponse(Timeout) ⇒
