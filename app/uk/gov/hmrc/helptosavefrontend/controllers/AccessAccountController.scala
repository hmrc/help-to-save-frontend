...
class AccessAccountController @Inject() (
    ...
    errorTemplate:           error_template)(implicit val transformer: NINOLogMessageTransformer,
  ...
  private def redirectToAccountHolderPage(pageURL: String)(
      ...
      def storeAttemptedRedirectThenRedirect(redirectTo: String): Future[Result] =
        sessionStore
          .store(
            HTSSession.empty.copy(attemptedAccountHolderPageURL = Some(pageURL)))
          .value
          .map {
            _.fold[Result]({ e ⇒
    ...
    checkIfEnrolled(
      {
        // not enrolled
        () ⇒
          storeAttemptedRedirectThenRedirect(
            routes.AccessAccountController.getNoAccountPage().url)
      }, {
        // enrolment check error
        e ⇒
          logger.warn(
            s"Could not check enrolment ($e) - proceeding to check eligibility",
            htsContext.nino)
          storeAttemptedRedirectThenRedirect(
            routes.EligibilityCheckController.getCheckEligibility().url)
      }, { () ⇒
